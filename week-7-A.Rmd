...
q_resamples <- q_resamples %>%
  mutate(hours_sampled = map(boot, ~ pull(., hours)),
         mean_hours = map_dbl(hours_sampled, ~ mean(., na.rm = TRUE)))
...
ggplot(aes(x = mean_hours,
  ...
  geom_vline(xintercept = mean(q_resamples$mean_hours, na.rm = T), col = "pink") +
  labs(x = "Mean Hours",
       y = "Probability",
       title = "Posterior Distribution for Mean Hours of Classes")
```
